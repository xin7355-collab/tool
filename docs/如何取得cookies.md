...
存進去幾筆、其中 youtube.com 的有幾筆。**不用自己另存 txt**。

> ⚠️ **Export 完要「馬上」執行**。中間只要複製過別的東西（網址、一段文字），
> 剪貼簿就被蓋掉了。讀到錯的東西時它會把實際讀到的前 40 個字印出來，
> 一看就知道是不是拿錯。

### 剪貼簿老是被蓋掉？改用檔案

把 cookies 貼進備忘錄／文字編輯器存成 `.txt`（放進「檔案」App 的
a-Shell → Documents 最省事），然後：

```
python3 yt2deck.py --set-cookies cookies.txt
```

也可以給完整路徑。檔案路徑不用擔心剪貼簿被蓋掉。

> **不要**用「直接貼到提示字元」的方式。cookies 的欄位是 **tab** 分隔的，
> 終端機會把 tab 當成自動補齊吃掉，貼進去就毀了。所以只走剪貼簿或檔案這兩條。
...
```
yt2deck v7（yt-dlp 2026.07.04，有 cookies）— 這次收到 20 支
